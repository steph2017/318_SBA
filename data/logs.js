...
const logs = [
    {
        ...
        date: "20230815",
        food_ids: [1, 4],
        tCals: food_ids.map((x) => foods.find(food => food.id === x)).cals.reduce((a, c) => (a + c, 0)), //takes food_ids , converts to the calories of the referenced food, then sums all the values
        ...
        tgFat: food_ids.map((x) => foods.find(food => food.id === x)).gfat.reduce((a, c) => (a + c, 0)),
        metcalTarget: (users.find(user => user.id === user_id).tarCals > tCals) ? false : true,
        calsLeft: users.find(user => user.id === user_id).tarCals - tCals
    },
    {
        id: 2,
        user_id: 1,
